...
#ifdef _WIN64 || _WIN32
#pragma error("Using Windows Native API path for MinGW/GNU toolchain")
        {
            // Use Win32 API calls that work with MinGW/GNU toolchain
            Sleep(500); // Instead of std::this_thread::sleep_for
            GetProfiler().RequestShutdown();
            while (!GetProfiler().HasShutdownFinished())
            {
                Sleep(10); // Instead of std::this_thread::sleep_for
            }
        }
#else
#pragma error("Using standard C++ path")
        {
...
#endif
          ,
          m_paramCallback(nullptr), m_sourceCallback(nullptr), m_queryImage(nullptr), m_queryData(nullptr), m_crashHandlerInstalled(false), m_programName(nullptr)
    {
        assert(!s_instance);
        s_instance = this;
...
#error Need to specify TRACY_CLIENT_ADDRESS for a QNX target.
    ...
    void Profiler::ClearQueues(moodycamel::ConsumerToken &token)
    {
        for (;;)
        {
            const auto sz = GetQueue().try_dequeue_bulk_single(token, [](const uint64_t &) {}, [](QueueItem *item, size_t sz)
                                                               { assert( sz > 0 ); while( sz-- > 0 ) FreeAssociatedMemory( *item++ ); });
            if (sz == 0)
                break;
        }
    ...
    Profiler::DequeueStatus Profiler::Dequeue(moodycamel::ConsumerToken &token)
    {
        ...
        const auto sz = GetQueue().try_dequeue_bulk_single(token, [this, &connectionLost](const uint32_t &threadId)
                                                           {
            if( ThreadCtxCheck( threadId ) == ThreadCtxStatus::ConnectionLost ) connectionLost = true; }, [this, &connectionLost](QueueItem *item, size_t sz)
                                                           {
